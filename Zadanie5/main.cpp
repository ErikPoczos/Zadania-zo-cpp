...
void siftUp(int data[], const size_t addIndex) {
    ...
    size_t iParent = (addIndex-1)/2;        //zistenie rodica
    if (data[iParent] > data[addIndex]) {   //ak je rodic vacsi ako jeho potomok
        swap(&data[iParent], &data[addIndex]);  //vymen
        siftUp(data, iParent);      //rekurzia
    }
}
...
void buildHeapSiftUp(int data[], const size_t length)
{
    ...
    for (size_t i = 0; i < length; i++){  //prechadzame heapom zhora nadol
        siftUp(data, i);
    }
}
...
{
    size_t Swap = topIndex;         //kandidat an vymenu
    size_t iLchild = topIndex*2+1;  //index laveho potomka
    size_t iRchild = topIndex*2+2;  //index praveho potomka

    if (iLchild >= length) {    //ak neexistuje lavy potomok
        return;                 //skonci
    }

    if (data[iLchild] < data[topIndex]) {   //je lavy potomok mensi ako koren?
        Swap = iLchild;                    //uloz si index ako kandidata na vymenu
    }

    if (iRchild < length && data[iRchild] < data[Swap]) {  //ak existuje pravy potomok a je mensi ako kandidat na vymenu
        Swap = iRchild;                                    //prepis kandidata
    }

    if (Swap != topIndex) {                                //ak mame kandidata
        swap(&data[topIndex], &data[Swap]);    // daj ho ako vrchol podstromu
        siftDown(data, Swap, length);             // rekurzia s novym vrcholom (novy podstrom)
    }
}
...
void buildHeapSiftDown(int data[], const size_t length) {
    size_t i = (length/2)-1;    //rozdelenie stromu na polovicu
    //size_t je unasigned typ premennej, v pripade, kedy je i=0 a odcitame jednotku, namiesto -1 sa i nastavi na najvacsiu hodnotu aku moze size_t mat (SIZE_MAX)
    //v tom pripade sme sa dostali na koren stromu, preto cyklus dalej nema pokracovat
    for (; i != SIZE_MAX; i--) {    //dekrementacia i je vlastne pohyb hore stromom
...
void heapSort(int data[], const size_t length) {
    //vytvorime heap cez shiftdown, kedze ma mensiu zlozitost ako shiftup
    buildHeapSiftDown(data, length);

    for (size_t i = 1; i < length; i++){
        swap(data[0], data[length-i]);  //extrakcia minima
        siftDown(data, 0, length-i);    //rekonstrukcia heapu
    }
}
